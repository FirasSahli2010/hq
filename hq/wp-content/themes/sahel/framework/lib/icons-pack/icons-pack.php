...
foreach ( glob( ELATED_FRAMEWORK_ICONS_ROOT_DIR . '/*/load.php' ) as $module_load ) {
	if ( sahel_elated_is_customizer_item_enabled( $module_load, 'sahel_performance_disable_icon_pack_' ) ) {
		include_once $module_load;
	}
}

if ( ! function_exists( 'sahel_elated_get_icon_pack_collections' ) ) {
	function sahel_elated_get_icon_pack_collections() {
		$options = apply_filters( 'sahel_elated_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'sahel_elated_get_icon_pack_collections' );
}

if ( ! function_exists( 'sahel_elated_get_icon_pack_options' ) ) {
	function sahel_elated_get_icon_pack_options() {
		$options = apply_filters( 'sahel_elated_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'sahel_elated_get_icon_pack_options' );
}

if ( ! function_exists( 'sahel_elated_get_icon_pack_social_options' ) ) {
	function sahel_elated_get_icon_pack_social_options() {
		$options = apply_filters( 'sahel_elated_filter_add_icon_pack_into_social_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'sahel_elated_get_icon_pack_social_options' );
}

/*
 * Class: SahelElatedClassIconCollections
 */
class SahelElatedClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = sahel_elated_get_icon_pack_collections();
		$this->iconPackOptions       = sahel_elated_get_icon_pack_options();
		$this->iconPackSocialOptions = sahel_elated_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = sahel_elated_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'sahel' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'sahel' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'sahel' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'sahel' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'sahel' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'sahel' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'sahel' )       => 'fa-lg',
			esc_html__( 'Small', 'sahel' )      => 'fa-2x',
			esc_html__( 'Medium', 'sahel' )     => 'fa-3x',
			esc_html__( 'Large', 'sahel' )      => 'fa-4x',
			esc_html__( 'Very Large', 'sahel' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iSahelElatedInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'sahel' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'sahel' ) ] = '';
