...
/*
   Class: SahelElatedClassFramework
...
class SahelElatedClassFramework {
	...
	private function __construct() {
		$this->eltdOptions         = SahelElatedClassOptions::get_instance();
		$this->eltdMetaBoxes       = SahelElatedClassMetaBoxes::get_instance();
		$this->eltdTaxonomyOptions = SahelElatedClassTaxonomyOptions::get_instance();
		$this->eltdUserOptions     = SahelElatedClassUserOptions::get_instance();
		$this->eltdDashboardOptions = SahelElatedClassDashboardOptions::get_instance();
	}
	...
	public function setSkin( SahelElatedClassSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class SahelElatedClassSkinManager
 *
...
class SahelElatedClassSkinManager {
	...
	/**
	 * @see SahelElatedClassSkinManager::setSkin()
	 */
	...
	private function setSkin( $skinName = ELATED_PROFILE_SLUG ) {
		if ( $skinName !== '' ) {
			if ( file_exists( get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php' ) ) {
				...
				$skinName = ucfirst( $skinName ) . esc_html__( 'Skin', 'sahel' );
...
/**
 * Class SahelElatedClassSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class SahelElatedClassSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( sahel_elated_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.eltdSCIcon = '<?php echo sahel_elated_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.eltdSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'sahel' ) ?>';
		</script>
	<?php }
...
class SahelElatedClassOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $sahel_elated_global_options ) ) {
			return $sahel_elated_global_options[ $key ];
...
/*
   Class: SahelElatedClassAdminPage
   A class that initializes Elated Admin Page
*/
class SahelElatedClassAdminPage implements iSahelElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title_label = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new SahelElatedClassFieldFactory();
		$this->slug    = $slug;
	...
	public function renderChild( iSahelElatedInterfaceRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: SahelElatedClassMetaBoxes
...
/*
   Class: SahelElatedClassMetaBox
   A class that initializes Elated Meta Box
*/
class SahelElatedClassMetaBox implements iSahelElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $sahel_elated_global_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: SahelElatedClassTaxonomyOptions
   A class that initializes SahelElatedClass Taxonomy Options
*/
class SahelElatedClassTaxonomyOptions {
	private static $instance;
...
/*
   Class: SahelElatedClassTaxonomyOption
   A class that initializes SahelElatedClass Taxonomy Option
*/
class SahelElatedClassTaxonomyOption implements iSahelElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new SahelElatedClassTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: SahelElatedClassUserOptions
   A class that initializes SahelElatedClass User Options
*/
class SahelElatedClassUserOptions {
	private static $instance;
	public $userOptions;
...
/*
   Class: SahelElatedClassUserOption
   A class that initializes SahelElatedClass User Option
*/
class SahelElatedClassUserOption implements iSahelElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new SahelElatedClassUserFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: SahelElatedClassDashboardOptions
   A class that initializes SahelElatedClass Dashboard Options
*/
class SahelElatedClassDashboardOptions {
	private static $instance;
...
/*
   Class: SahelElatedClassDashboardOption
   A class that initializes SahelElatedClass Dashboard Option
*/
class SahelElatedClassDashboardOption implements iSahelElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	
	function __construct() {
		$this->layout        = array();
		$this->factory       = new SahelElatedClassDashboardFieldFactory();
	}
...
if ( ! function_exists( 'sahel_elated_init_framework_variable' ) ) {
	function sahel_elated_init_framework_variable() {
		global $sahel_elated_global_Framework;
		
		$sahel_elated_global_Framework = SahelElatedClassFramework::get_instance();
		$eltdSkinManager                 = new SahelElatedClassSkinManager();
		$sahel_elated_global_Framework->setSkin( $eltdSkinManager->getSkin() );
	}
	
	add_action( 'sahel_elated_action_before_options_map', 'sahel_elated_init_framework_variable' );
}
?>