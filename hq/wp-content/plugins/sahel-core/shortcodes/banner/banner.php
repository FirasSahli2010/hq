<?php
namespace SahelCore\CPT\Shortcodes\Banner;
...
class Banner implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Banner', 'sahel-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						...
						array(
							...
							'heading'    => esc_html__( 'Image Overlay Color', 'sahel-core' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'hover_behavior',
							'heading'     => esc_html__( 'Hover Behavior', 'sahel-core' ),
							'value'       => array(
								esc_html__( 'Visible on Hover', 'sahel-core' )   => 'eltdf-visible-on-hover',
								esc_html__( 'Visible on Default', 'sahel-core' ) => 'eltdf-visible-on-default',
								esc_html__( 'Disabled', 'sahel-core' )           => 'eltdf-disabled'
							),
							'save_always' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Info Position', 'sahel-core' ),
							'value'       => array(
								esc_html__( 'Default', 'sahel-core' )  => 'default',
								esc_html__( 'Centered', 'sahel-core' ) => 'centered'
							),
							'save_always' => true
						),
						array(
							'type'       => 'attach_image',
							'param_name' => 'info_content_background_image',
							'heading'    => esc_html__( 'Info Content Background Image', 'sahel-core' )
						),
						array(
							'type'        => 'textfield',
							'param_name'  => 'info_content_padding',
							'heading'     => esc_html__( 'Info Content Padding', 'sahel-core' ),
							'description' => esc_html__( 'Please insert padding in format top right bottom left', 'sahel-core' )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'subtitle',
						...
						array(
							...
							'heading'    => esc_html__( 'Link Text', 'sahel-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						),
						array(
							'type'       => 'colorpicker',
							'param_name' => 'link_color',
							'heading'    => esc_html__( 'Link Text Color', 'sahel-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'link_top_margin',
							'heading'    => esc_html__( 'Link Text Top Margin (px)', 'sahel-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						)
					)
				)
	...
	public function render( $atts, $content = null ) {
		...
		$html = sahel_core_get_shortcode_module_template_part( 'templates/banner', 'banner', '', $params );
	...
	private function getOverlayStyles( $params ) {
		...
		if ( ! empty( $params['info_content_background_image'] ) ) {
			$styles[] = 'background-image: url(' . wp_get_attachment_url( $params['info_content_background_image'] ) . ')';
		}
	...
	private function getLinkStyles( $params ) {
		...
		if ( ! empty( $params['link_top_margin'] ) ) {
			$styles[] = 'margin-top: ' . sahel_elated_filter_px( $params['link_top_margin'] ) . 'px';
		}
